  ...
  return (
    <div>
      ...
      <header className="bg-[#0F2242] text-white px-5 pt-10 pb-6">
        ...
        <h1 className="text-[18px] font-extrabold mt-1">{resident.villaName}</h1>
        <p className="text-[12px] text-white/60 mt-0.5">{resident.ho} · {resident.name}</p>
      </header>
      ...
      <div className="px-5 pt-5 max-w-screen-sm mx-auto">
        ...
        {currentMonth ? (
          <div className="bg-white rounded-2xl p-5 border border-[#E8EBF0] shadow-sm">
            <div className="flex items-start justify-between mb-3">
              <div>
                <p className="text-[12px] text-[#6B7280] font-bold">{currentMonth.year_month}</p>
                <h2 className="text-[16px] font-extrabold text-[#0F2242] mt-0.5">
              ...
              {(() => {
                ...
                if (pay?.is_paid) {
                  return <span className="bg-[rgba(46,204,113,0.12)] text-[#2ECC71] text-[11px] font-bold px-2.5 py-1 rounded-full">납부완료</span>;
                }
                return <span className="bg-[rgba(231,76,60,0.12)] text-[#E74C3C] text-[11px] font-bold px-2.5 py-1 rounded-full">미납</span>;
              })()}
            </div>
            <p className="text-[32px] font-black text-[#4263E8]">₩{fmt(myAmount(currentMonth))}</p>
              ...
              if (pay?.is_paid && pay.paid_at) {
                return (
                  <div className="mt-3 pt-3 border-t border-[#E8EBF0] flex items-center justify-between">
                    ...
                    <span className="text-[12px] text-[#0F2242] font-semibold">
            ...
            {currentMonth.bill_items.length > 0 && (
              <div className="mt-4 pt-4 border-t border-[#E8EBF0]">
                <p className="text-[11px] text-[#6B7280] font-bold mb-2">전체 항목</p>
                ...
                <div className="flex justify-between pt-2 mt-2 border-t border-[#E8EBF0]">
                  <span className="text-[12px] text-[#6B7280] font-bold">빌라 전체 합계</span>
                  <span className="text-[13px] text-[#0F2242] font-extrabold">
                    ₩{fmt(currentMonth.bill_items.reduce((s, i) => s + i.amount, 0))}
                  </span>
                </div>
                <div className="flex justify-between pt-1">
                  <span className="text-[12px] text-[#4263E8] font-bold">내 호실 분담</span>
                  <span className="text-[13px] text-[#4263E8] font-extrabold">
          ...
          <div className="bg-white rounded-2xl p-8 border border-[#E8EBF0] text-center">
            ...
            <p className="text-[14px] font-bold text-[#0F2242]">이번 달 발행된 청구서가 없습니다</p>
        ...
        {reserveAccumulated > 0 && (
          <div className="mt-3 bg-gradient-to-br from-[#FFF8EE] to-[#FFEFD8] border border-[rgba(243,156,18,0.3)] rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-[12px] font-bold text-[#9C6F1F]">수선충당금 누적</span>
            </div>
            <p className="text-[20px] font-black text-[#9C6F1F]">₩{fmt(reserveAccumulated)}</p>
            <p className="text-[11px] text-[#9C6F1F]/70 mt-1">
        ...
        {(villa?.account_bank || villa?.account_number) && (
          <div className="mt-3 bg-[#EEF1FB] border border-[#4263E8]/15 rounded-2xl p-4">
            <p className="text-[11px] font-bold text-[#4263E8] mb-1">💳 관리비 입금 계좌</p>
            <p className="text-[14px] font-bold text-[#0F2242]">
              {villa.account_bank} {villa.account_number}
            </p>
            {villa.account_holder && (
              <p className="text-[11px] text-[#6B7280] mt-0.5">예금주: {villa.account_holder}</p>
            )}
          </div>
        )}
        ...
        {pastMonths.length > 0 && (
          <div className="mt-5">
            <button
              ...
              className="w-full flex items-center justify-between text-[13px] text-[#6B7280] font-bold mb-2.5"
            >
                ...
                {pastMonths.map(m => {
                  ...
                  return (
                    <div key={m.id} className="bg-white border border-[#E8EBF0] rounded-xl p-3.5 flex items-center justify-between">
                      <div>
                        <p className="text-[12px] text-[#6B7280] font-bold">{m.year_month}</p>
                        <p className="text-[13px] text-[#0F2242] font-bold mt-0.5">{m.label ?? `${m.year_month} 관리비`}</p>
                      </div>
