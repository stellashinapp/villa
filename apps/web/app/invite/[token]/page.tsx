  ...
  if (error || !inv) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#F5F6FA] px-5">
        ...
        <Link href="/" className="bg-[#3766EE] text-white px-6 py-3 rounded-xl font-bold">홈으로</Link>
      </div>
    );
  }
      ...
      <div className="max-w-sm mx-auto">
        ...
        <form onSubmit={accept} className="bg-white border border-[#E8EBF0] rounded-2xl p-5 shadow-sm space-y-3">
          ...
          <input
            ...
            maxLength={11}
            inputMode="tel"
            className="w-full bg-white border border-[#E8EBF0] rounded-xl px-3 py-3 text-[16px] outline-none focus:border-[#3766EE]"
            required
          />
          <button
            type="submit"
            disabled={accepting}
            className="w-full bg-[#3766EE] text-white py-3.5 rounded-xl text-[16px] font-bold disabled:opacity-50"
          >
