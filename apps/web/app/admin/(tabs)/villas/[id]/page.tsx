  ...
  return (
    <div className="px-5 pt-6 pb-8 max-w-screen-sm mx-auto">
      ...
      <div className="mt-3 mb-5">
        {editingBasic ? (
          <div className="bg-white border border-[#6C2FF2] border-[1.5px] rounded-2xl p-4 shadow-sm space-y-3">
            <div>
              ...
              <input value={editName} onChange={e => setEditName(e.target.value)} maxLength={50}
                className="w-full bg-white border border-[#E8EBF0] rounded-2xl px-3 py-2.5 text-[16px] outline-none focus:border-[#6C2FF2]" />
            </div>
            <div>
            ...
            <div className="flex gap-2">
              <button onClick={saveBasic} disabled={savingBasic} className="flex-1 bg-[#6C2FF2] text-white py-2.5 rounded-2xl text-[16px] font-bold disabled:opacity-50">
                {savingBasic ? '저장 중…' : '저장'}
              </button>
              <button onClick={() => setEditingBasic(false)} className="px-4 bg-[#F5F6FA] text-[#6B7280] py-2.5 rounded-2xl text-[16px] font-bold">취소</button>
            </div>
          </div>
        ) : (
          <div className="flex items-start justify-between">
            ...
            <button onClick={startEditBasic} className="text-[15px] text-[#6C2FF2] font-bold ml-3 flex-shrink-0 mt-1">✏️ 편집</button>
          </div>
        )}
      </div>
      ...
      {status.current_bm_id ? (
        <div className="bg-gradient-to-br from-[#F1ECFE] to-[#F8FAFF] border border-[#6C2FF2]/20 rounded-2xl p-4 shadow-sm">
          ...
          <div className="flex items-end justify-between mb-3">
            <div>
              <p className={`text-[38px] font-black ${status.current_pay_rate >= 80 ? 'text-[#2ECC71]' : 'text-[#6C2FF2]'}`}>
          ...
          {unpaidCount > 0 && (
            <button
              ...
              className="w-full bg-[#6C2FF2] text-white py-2.5 rounded-2xl text-[15px] font-bold mb-2 hover:bg-[#5320C9] transition"
            >
              📢 미납세대 독촉 보내기
            </button>
          )}
          <Link href={`/admin/villas/${villa.id}/bills`} className="block w-full bg-white border border-[#E8EBF0] text-[#6C2FF2] py-2.5 rounded-2xl text-[15px] font-bold text-center">
            세대별 납부 현황 상세 →
          </Link>
        </div>
      ) : (
        <div className="bg-[#F5F6FA] border border-dashed border-[#E8EBF0] rounded-2xl p-5 text-center">
          ...
          <Link href={`/admin/villas/${villa.id}/bills`} className="block w-full bg-[#6C2FF2] text-white py-3 rounded-2xl text-[15px] font-bold hover:bg-[#5320C9] transition">
            관리비 발행
          </Link>
        </div>
      )}
        ...
        <div className="mt-3">
          ...
          <Link href={`/admin/villas/${villa.id}/messages`} className="block bg-white border border-[#FF3B30]/30 rounded-2xl p-4">
            <div className="flex items-center justify-between">
              ...
              <button className="bg-[#6C2FF2] text-white px-4 py-2.5 rounded-2xl text-[15px] font-bold">바로 확인</button>
            </div>
          </Link>
        </div>
      )}
      ...
      <div className="flex items-center justify-between mt-6 mb-2">
        ...
        {!editingAccount && (
          <button onClick={startEdit} className="text-[14px] text-[#6C2FF2] font-bold">
        ...
        <div className="bg-white border border-[#6C2FF2] border-[1.5px] rounded-2xl p-4 shadow-sm space-y-3">
          <div className="grid grid-cols-2 gap-2.5">
            <div>
              ...
              <select value={bank} onChange={e => setBank(e.target.value)} className="w-full bg-white border border-[#E8EBF0] rounded-2xl px-3 py-2.5 text-[16px] outline-none focus:border-[#6C2FF2]">
            ...
            <div>
              ...
              <input value={holder} onChange={e => setHolder(e.target.value)} maxLength={30} className="w-full bg-white border border-[#E8EBF0] rounded-2xl px-3 py-2.5 text-[16px] outline-none focus:border-[#6C2FF2]" />
            </div>
          </div>
          <div>
            <label className="block text-[14px] font-bold text-[#6B7280] mb-1.5">계좌번호</label>
            <input value={number} onChange={e => setNumber(e.target.value)} maxLength={30} className="w-full bg-white border border-[#E8EBF0] rounded-2xl px-3 py-2.5 text-[16px] outline-none focus:border-[#6C2FF2]" />
          </div>
          <div className="flex gap-2">
            <button onClick={saveAccount} disabled={savingAccount} className="flex-1 bg-[#6C2FF2] text-white py-2.5 rounded-2xl text-[16px] font-bold disabled:opacity-50">
              {savingAccount ? '저장 중…' : '저장'}
            </button>
            <button onClick={() => setEditingAccount(false)} className="px-4 bg-[#F5F6FA] text-[#6B7280] py-2.5 rounded-2xl text-[16px] font-bold">취소</button>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between mt-6 mb-2">
        ...
        {!editingExpose && (
          <button onClick={startEditExpose} className="text-[14px] text-[#6C2FF2] font-bold">수정</button>
        )}
      </div>
      {editingExpose ? (
        <div className="bg-white border-[1.5px] border-[#6C2FF2] rounded-2xl p-4 shadow-sm space-y-3">
          <label className="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" checked={exposeContact} onChange={e => setExposeContact(e.target.checked)} className="mt-0.5 w-5 h-5 flex-shrink-0 accent-[#6C2FF2]" />
            <span>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} maxLength={300}
              ...
              className="w-full bg-white border border-[#E8EBF0] rounded-2xl px-3 py-2.5 text-[15px] outline-none focus:border-[#6C2FF2] resize-none" />
            <p className="text-[12px] text-[#9CA3AF] mt-1">{notes.length}/300</p>
          </div>
          <div className="flex gap-2">
            <button onClick={saveExpose} disabled={savingExpose} className="flex-1 bg-[#6C2FF2] text-white py-2.5 rounded-2xl text-[16px] font-bold disabled:opacity-50">
              {savingExpose ? '저장 중…' : '저장'}
            </button>
            <button onClick={() => setEditingExpose(false)} className="px-4 bg-[#F5F6FA] text-[#6B7280] py-2.5 rounded-2xl text-[16px] font-bold">취소</button>
          </div>
        </div>
      ) : (
        <div className="bg-white border border-[#E8EBF0] rounded-2xl p-4 shadow-sm space-y-2.5">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-[13px] font-bold px-2 py-0.5 rounded-full ${villa.expose_admin_contact ? 'bg-[#F1ECFE] text-[#6C2FF2]' : 'bg-[#F5F6FA] text-[#9CA3AF]'}`}>
...
function SubMenu({ href, icon, label, hint }: { href: string; icon: IconName; label: string; hint: string }) {
  return (
    <Link href={href} className="flex items-center bg-white border border-[#E8EBF0] rounded-2xl p-4 shadow-sm active:scale-[0.99] transition">
      <span className="mr-3 flex-shrink-0 w-10 h-10 rounded-2xl bg-[#F1ECFE] flex items-center justify-center">
        <Icon name={icon} size={22} color="#6C2FF2" filled />
      </span>
