  ...
  return (
    <div className="px-5 pt-6 pb-8 max-w-screen-sm mx-auto">
      ...
      <div className="flex justify-between items-end mt-3 mb-5">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-[#3766EE] text-white text-[15px] font-bold px-3.5 py-2 rounded-2xl">
          {showForm ? '취소' : '＋ 공지 작성'}
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={submit} className="mb-4 bg-white border border-[#E8EBF0] rounded-2xl p-4 shadow-sm space-y-3">
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="제목" maxLength={50}
            className="w-full bg-white border border-[#E8EBF0] rounded-2xl px-3 py-2.5 text-sm outline-none focus:border-[#3766EE]" required />
          <textarea value={body} onChange={e => setBody(e.target.value)} placeholder="내용" rows={5} maxLength={2000}
            className="w-full bg-white border border-[#E8EBF0] rounded-2xl px-3 py-2.5 text-sm outline-none focus:border-[#3766EE] resize-none" required />
          ...
          <button type="submit" disabled={submitting} className="w-full bg-[#3766EE] text-white py-2.5 rounded-2xl text-[16px] font-bold disabled:opacity-50">
      ...
      {loading ? <p className="text-center text-sm text-[#9CA3AF] mt-10">불러오는 중…</p>
        : notices.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {notices.map(n => (
              <div key={n.id} className={`bg-white rounded-2xl p-4 border shadow-sm ${n.is_pinned ? 'border-[#FF6B35] border-[1.5px]' : 'border-[#E8EBF0]'}`}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button onClick={() => togglePin(n)} className="ml-auto text-[14px] text-[#3766EE] font-bold hover:underline">
