  ...
  return (
    <div className="px-5 pt-6 pb-8 max-w-screen-sm mx-auto">
      <header className="mb-5">
        <p className="text-[12px] text-[#4263E8] tracking-widest font-bold mb-1">VILLATOLK ADMIN</p>
        <h1 className="text-[24px] font-black text-[#0F2242] leading-tight">{profile?.name || profile?.email}님</h1>
      ...
      {hasAlerts && (
        <div className="mb-5">
          <p className="text-[12px] text-[#6B7280] font-bold tracking-widest mb-2">오늘 처리 필요</p>
          <div className="space-y-2">
            {agg.pendingApplications > 0 && (
              <Link href="/admin/applications" className="block bg-[#F39C12] text-white rounded-xl px-4 py-3.5 active:scale-[0.99] transition">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[12px] font-bold opacity-80 tracking-widest">📮 가입 신청 대기</p>
                    <p className="text-[18px] font-extrabold mt-0.5">{agg.pendingApplications}건 처리 필요</p>
                  </div>
            ...
            {agg.pendingMoveouts > 0 && (
              <Link href="/admin/villas" className="block bg-[#FF6B35] text-white rounded-xl px-4 py-3.5 active:scale-[0.99] transition">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[12px] font-bold opacity-80 tracking-widest">📦 이주 확정 대기</p>
                    <p className="text-[18px] font-extrabold mt-0.5">{agg.pendingMoveouts}건 확정 필요</p>
                  </div>
                  <span className="text-xl">›</span>
                </div>
              </Link>
            ...
            {agg.unreadMessages > 0 && (
              <Link href="/admin/inbox" className="block bg-[#E74C3C] text-white rounded-xl px-4 py-3.5 active:scale-[0.99] transition">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[12px] font-bold opacity-80 tracking-widest">✉️ 미답변 메시지</p>
                    <p className="text-[18px] font-extrabold mt-0.5">{agg.unreadMessages}건 응답 필요</p>
                  </div>
                  <span className="text-xl">›</span>
                </div>
              </Link>
            ...
            {agg.unpaidCurrentMonth > 0 && (
              <div className="bg-white border border-[rgba(231,76,60,0.3)] rounded-xl px-4 py-3.5">
                <p className="text-[12px] font-bold text-[#E74C3C] tracking-widest">💸 이번 달 미납</p>
                <p className="text-[18px] font-extrabold mt-0.5 text-[#E74C3C]">{agg.unpaidCurrentMonth}세대 미납</p>
                <p className="text-[12px] text-[#6B7280] mt-0.5">청구한 빌라 {agg.publishedThisMonth}개 합산 · 빌라별 상세는 아래 카드</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between mb-2">
        <p className="text-[12px] text-[#6B7280] font-bold tracking-widest">내 빌라</p>
        <Link href="/admin/villas/add" className="text-[13px] text-[#4263E8] font-bold">＋ 빌라 추가</Link>
      </div>
      <div className="space-y-3">
        {villas.length === 0 ? (
          <Link href="/admin/villas/add" className="block bg-white border border-dashed border-[#4263E8]/30 rounded-2xl p-6 text-center hover:bg-[#EEF1FB]">
            ...
            <p className="text-[15px] font-bold text-[#0F2242]">첫 빌라를 등록해주세요</p>
            <p className="text-[12px] text-[#6B7280] mt-1">청구·입주민·공지 관리 시작</p>
          </Link>
        ) : (
          villas.map(v => (
            <Link
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[12px] text-[#6B7280] mt-0.5 truncate">{v.address} · {v.total_units}세대</p>
                </div>
              ...
              {v.current_month_label ? (
                <div className="bg-[#EEF1FB] border border-[#4263E8]/15 rounded-xl p-3 mt-2">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-[11px] text-[#6B7280] font-bold">{v.current_month_label}</p>
                      <p className="text-[18px] font-extrabold text-[#0F2242] mt-0.5">
                        ₩{fmt(v.per_unit_amount)} <span className="text-[12px] text-[#6B7280] font-normal">세대당 · {v.total_units}세대</span>
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-[11px] text-[#6B7280] font-bold">납부율</p>
                      <p className={`text-[18px] font-extrabold ${v.pay_rate >= 80 ? 'text-[#2ECC71]' : v.pay_rate >= 50 ? 'text-[#F39C12]' : 'text-[#E74C3C]'}`}>
                        {v.pay_rate}%
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-[#F5F6FA] border border-dashed border-[#E8EBF0] rounded-xl p-3 mt-2 text-center">
                  <p className="text-[12px] text-[#9CA3AF]">이번 달 관리비 미발행 — 빌라 클릭 → 관리비 메뉴</p>
                </div>
              )}
            </Link>
          ))
      ...
      <div className="mt-8">
        <p className="text-[12px] text-[#6B7280] font-bold tracking-widest mb-2">구독</p>
        <div className={`rounded-2xl p-4 ${
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-[12px] font-bold tracking-widest ${sub?.status === 'active' ? 'opacity-80' : 'text-[#6B7280]'}`}>구독 상태</p>
              <p className={`text-[18px] font-extrabold mt-0.5 ${sub?.status === 'past_due' ? 'text-[#E74C3C]' : sub?.status === 'trialing' ? 'text-[#F39C12]' : ''}`}>{subLabel}</p>
              {sub?.card_last4 && (
                <p className={`text-[13px] mt-1 ${sub?.status === 'active' ? 'opacity-90' : 'text-[#6B7280]'}`}>
                  {sub.card_brand} ····{sub.card_last4}
                </p>
              )}
              {!sub && <p className="text-[13px] text-[#6B7280] mt-1">카드 등록 시 무료 체험 시작</p>}
            </div>
            <Link href="/admin/settings" className={`text-[12px] font-bold ${sub?.status === 'active' ? 'text-white/80' : 'text-[#4263E8]'}`}>
              관리 →
            </Link>
          </div>
        </div>
